...
module.exports = {
	...
	async execute(client, log, [r, u], {config, Ticket, Setting}) {
			...
			try {
				return dm.send(
					new MessageEmbed()
						...
						.setTitle(`❌ **Vous avez déjà ${tickets.count} ou plus de tickets ouvert**`)
						.setDescription(`Utilisez \`${config.prefix}close\` dans un ticket inutile pour le fermer.\n\n${ticketList.join(',\n')}`)
						.setFooter(channel.guild.name, channel.guild.iconURL())
				);
			} catch (e) {
				let m = await channel.send(
					new MessageEmbed()
						...
						.setTitle(`❌ **Vous avez déjà ${tickets.count} ou plus de tickets ouvert**`)
						.setDescription(`Utilisez \`${config.prefix}close\` dans un ticket inutile pour le fermer.\n\n${ticketList.join(',\n')}`)
						.setFooter(channel.guild.name + ' | Ce message va être supprimé dans 15 secondes', channel.guild.iconURL())
				);
		...
		channel.guild.channels.create(name, {
			...
			await c.send(ping + `${u} a créé un ticket`);
			...
			if (config.logs.discord.enabled)
				client.channels.cache.get(config.logs.discord.channel).send(
					new MessageEmbed()
						.setColor(config.colour)
						.setAuthor(u.username, u.displayAvatarURL())
						.setTitle('Nouveau ticket (via le message dans #créer-un-ticket)')
						.setDescription(`\`${topic}\``)
						.addField('Créateur', u, true)
						.addField('Ticket', c, true)
						.setFooter(channel.guild.name, channel.guild.iconURL())
						.setTimestamp()
				);

			log.info(`${u.tag} à créé un ticket (#${name}) via le message dans #créer-un-ticket`);
		}).catch(log.error);
	}
};
