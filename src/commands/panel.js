...
module.exports = {
	name: 'panel',
	description: 'Crée un message pour créer les tickets',
	usage: '',
	aliases: ['widget'],
	args: false,
	...
	async execute(client, message, _args, log, {config, Setting}) {
		...
		if (!msgID) {
			...
			try {
				...
				if (panel) {
					panel.delete({ reason: 'Création d\'un nouveau message' }).then(() => log.info('Ancien message supprimé')).catch(e => log.warn(e)); // delete old panel
				}
			} catch (e) {
				log.warn('Impossible de supprimer l\'ancien message');
			}
		}
		...
		log.info(`${message.author.tag} a créé un nouveau message`);
	}
};
